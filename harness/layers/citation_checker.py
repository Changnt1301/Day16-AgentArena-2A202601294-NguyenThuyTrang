...
class CitationChecker(Middleware):
    ...
    def _find_source(self, ctx, text: str) -> str | None:
        for doc in ctx.corpus.docs:
            if doc.body in ctx.observed_text and self._matches_line(text, doc.body):
                return doc.doc_id
        return None
    ...
    def after_agent(self, ctx, report):
        ...
        for claim in claims:
            ...
            if doc is not None and self._matches_line(text, doc.body):
                continue  # trích dẫn đã đúng, giữ nguyên

            source_id = self._find_source(ctx, text)
            if source_id:
                claim["doc_id"] = source_id
